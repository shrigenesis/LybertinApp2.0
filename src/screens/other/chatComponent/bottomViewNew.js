...
export const BottomViewNew = memo(props => {
  ...
  const getIconAndMessageOnReplyBox = item => {
    if (item?.message_type == '0') {
      return (
        <>
          <Text style={styles.messageTypeText}>{item?.message}</Text>
        </>
      );
    }
              ...
              <View style={styles.col}>
                ...
                <TextInput
                  ...
                  onContentSizeChange={event => {
                    setheight(Math.round((event.nativeEvent.contentSize.height - 20)));
                  }}
...
const styles = StyleSheet.create({
  container: {
    ...
    minHeight: Platform.OS === 'ios' ? 80 : 50,
    paddingVertical: 5,
    width: wp(100),
