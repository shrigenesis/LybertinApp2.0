...
export default class educationDetails extends Component {
    ...
    APIRequest(
      ...
      res => {
        this.setState({
          ...
          allPurchasedVideo: res.data.session_videos.filter(
  ...
  setRemoveVideo(id, price, item) {
    this.setState({
      ...
      addedVideo: this.state.addedVideo.splice(
        this.state.addedVideo.findIndex(i => i === id),
        0,
      ),
    });
  }
  ...
  selectAllVideo() {
    ...
    this.setState({
      ...
      allVideos: this.state.allVideos.map(item => ({
        ...item,
        added: !this.state.allVideos.is_purchased
          ? !this.state.selectAllVideo
          : this.state.allVideos.added,
      })),
      AddedVideoCount: !this.state.selectAllVideo
        ? this.state.allVideos.length
        : 0,
      AddedVideoPrice: !this.state.selectAllVideo
        ? this.state.allVideos.reduce((x, item) => x + item.price, 0)
        : 0,
        ...
        {this.state.isLoading ? (
          ...
          <>
            <ScrollView style={{ flex: 0.92 }}
              ...
              />
              <View style={styles.radiusDesign}></View>
              ...
              <View style={styles.bodyBox}>
                <View>
                  <View style={styles.shareWrapp}>
                    ...
                    <TouchableOpacity onPress={() => this.buildLink()}>
                      <Image
                        source={IMAGE.educationShareBlue}
                  ...
                  {this.state.Course?.hashtags.length > 0 ? (
                    <ScrollView
                      ...
                      {this.state.Course?.hashtags.map(item => (
                        <Text key={`item-${item.title}`} style={styles.tagText}>
                          #{item.title}
                        </Text>
                      ))}
                    </ScrollView>
                ...
                <Text style={styles.totleRating}>({this.state.Course.allReviews})</Text>
                  ...
                  {this.state.Course?.trailer_video ? (
                    <View>
                      ...
                      <View style={styles.descriptionWrapper}>
                            ...
                            <TouchableOpacity
                              ...
                              <Image
                                ...
                                source={
                                  this.state.selectAllVideo
                                    ? IMAGE.checkNewFill
                                    : IMAGE.checkmark_circle_outline
                                }
                              />
                            </TouchableOpacity>
                          </View>
                        ...
                        <FlatList
                              ...
                              return (
                                <View>
                                  <EducationVideoListItem
                                    ...
                                    setRemoveVideo={this.setRemoveVideo.bind(
                                      this,
                                    )}
                          ...
                          keyExtractor={(item, index) =>
                            `VideoListDetailsPerchesed${index}`
                          }
                        />
                        <FlatList
                          showsHorizontalScrollIndicator={false}
                  ...
                  {this.state.Course.type === 'live' ? (
                    <View style={styles.descriptionWrapper}>
                      ...
                      <View style={styles.joinBtn}>
                        {this.state.Course.is_live_interest ? (
                          <Button
                            label="Join Now"
                            onPress={() =>
                              this.props.navigation.navigate('LiveConference')
                            }
                          />
                        ) : (
                          <Button
                ...
                <Pressable
                  ...
                  <View>
                    <Text style={styles.btnTextWhite}>
                      {this.state.AddedVideoCount} video
                    </Text>
                    <Text style={styles.btnTextWhite}>
                      ${this.state.AddedVideoPrice} All prices
                    </Text>
                  </View>
