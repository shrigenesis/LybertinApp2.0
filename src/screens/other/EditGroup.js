...
const EditGroup = ({ navigation, route }) => {
  ...
  const [groupPrivacy, setgroupPrivacy] = useState(route?.params?.groupPrivacy);
  const [MediaPrivacy, setMediaPrivacy] = useState(route?.params?.mediaPrivacy);
  const [groupName, setgroupName] = useState(route?.params?.groupName);
  const [groupDesc, setgroupDesc] = useState(route?.params?.groupDes);
  const [cover, setcover] = useState(route?.params?.coverImage);
  ...
  const fetchGroupDetail = () => {
    ...
    APIRequest(
      config,
      res => {
        console.log('Api response===', res.data.group_members);
        setcontactList(res.data.group_members);
      },
      err => {
        ...
        <View>
          ...
          <View>
            ...
            {/* <TextInput
              ...
              placeholderTextColor={'#0F2D52A6'}
              placeholder="Add Welcome push message"
              style={style.inputStyle}
            /> */}
          </View>
